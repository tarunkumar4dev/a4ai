...
const STATUS_STYLES: Record<Status, string> = {
  ...
  Invited: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-200",
  Suspended: "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-200",
};

function StatusPill({ status }: { status: Status }) {
  return (
    <Badge className={`${STATUS_STYLES[status]} font-medium px-2.5 py-0.5`}>{status}</Badge>
  );
}
...
export default function StudentsPage() {
  ...
  const [classFilter, setClassFilter] = useState<"All" | "Class 8" | "Class 9" | "Class 10">("All");
  ...
  const onInvite = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Here you can call your API / Supabase function
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      >
        <div>
          ...
          <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">Students</h1>
          <p className="text-sm text-muted-foreground">Manage enrolled students, invites, and statuses.</p>
        </div>
      ...
      <Card className="p-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="relative">
            ...
            <motion.span
              layoutId="underline"
              className="absolute left-3 right-3 bottom-0 h-px bg-gradient-to-r from-indigo-500/60 via-fuchsia-500/60 to-emerald-500/60"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-9 w-full rounded-md border border-border bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            >
              <option>All</option>
          ...
          <div className="grid grid-cols-3 items-center gap-3 rounded-md border border-dashed border-border p-2 text-center text-sm">
            ...
            <div>
              <div className="font-semibold">{MOCK.filter((x) => x.status === "Active").length}</div>
              <div className="text-xs text-muted-foreground">Active</div>
            </div>
            <div>
              <div className="font-semibold">{MOCK.filter((x) => x.status === "Invited").length}</div>
          ...
          <table className="min-w-full text-sm">
            <thead className="text-left text-muted-foreground">
              <tr className="border-b border-border/60">
                ...
                <Th onClick={() => toggleSort("className")} active={sortKey === "className"} dir={sortDir}>
                  Class
                </Th>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <div className="text-xs text-muted-foreground">Page {pageSafe} of {totalPages}</div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
      ...
      {/* Safety banner */}
      <div className="mt-4 flex items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-amber-800 dark:text-amber-200">
        <ShieldAlert className="h-4 w-4" />
        <p className="text-xs">
          Tip: You can wire this UI to Supabase. Create a `students` table and replace the MOCK array with data from an RPC/Edge Function.
        </p>
      </div>
    </div>
  );
...
/* =====================================================================
   Invite Dialog (headless — uses native <dialog>-like card)
   If you have shadcn <Dialog />, swap this with it.
===================================================================== */
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative z-10 w-[min(560px,92vw)] rounded-xl border border-border bg-background p-5 shadow-xl"
            >
              <div className="mb-4">
              ...
              <form onSubmit={(e) => { onSubmit(e); setOpen(false); }} className="grid gap-3">
                ...
                <div className="grid gap-1.5">
                  ...
                  <select name="className" defaultValue="Class 10" className="h-9 rounded-md border border-border bg-background px-3 text-sm">
                    <option>Class 8</option>
                    <option>Class 9</option>
                    <option>Class 10</option>
                  </select>
