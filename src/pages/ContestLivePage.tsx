...
export default function ContestLivePage() {
  ...
  // questions come from your hook (kept same API)
  ...
  const [contestGate, setContestGate] = useState<{ allowed: boolean; reason?: string }>({
    allowed: true
  });
  ...
  useEffect(() => {
    const gate = async () => {
      ...
      if (error) {
        ...
        setContestGate({ allowed: false, reason: "Server error. Try again." });
        return;
      }
      if (!data) {
        setContestGate({ allowed: false, reason: "Contest not found." });
        return;
      }
    ...
    gate();
    // poll lightly in case host toggles live
              ...
              <CardContent className="p-6">
                <div className="grid md:grid-cols-2 gap-8 py-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {!isTabActive && <div className="text-amber-700 text-sm">Warning: stay on this tab during the contest.</div>}
...
// ---- hook import placed at top; keeping for type safety
// If your editor complains, keep the original import.
