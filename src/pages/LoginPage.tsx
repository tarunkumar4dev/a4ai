...
const LoginPage = () => {
  ...
  /** Resolve the "next" page: from state.from OR ?next= OR /dashboard */
  const resolveNext = () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams: {
            // Use "consent" in dev to force refresh token,
            // but in prod you might prefer this:
      ...
      if (error) throw error;
      // Browser will redirect to Google → /auth/callback
    } catch (error) {
      toast({
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      {/* Right side with image/marketing */}
