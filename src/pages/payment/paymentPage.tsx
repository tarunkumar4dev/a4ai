...
export default function PaymentPage() {
  ...
  async function openRazorpay(amountPaise: number) {
    ...
    try {
      ...
      // 1) Create order (paise)
      const body: Record<string, any> = { amount: amountPaise, payment_method: selectedMethod };
      if (selectedMethod === "upi" && import.meta.env.MODE !== "production") body.vpa = "success@razorpay";
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            ...
            if (verifyRes.ok && result?.success) setPaymentSuccess(true);
            else throw new Error(result?.error || "Payment Verification Failed");
          } catch (err: any) {
    ...
    return (
      <div className="min-h-[100svh] bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center p-4 sm:p-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 sm:p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300 mb-6">Thank you for your subscription. Your payment has been processed successfully.</p>
          <button onClick={() => (window.location.href = "/dashboard")} className="w-full rounded-full bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold py-3.5 sm:py-4 px-6 shadow-md focus:outline-none focus:ring-2 focus:ring-blue-400/70 transition">
            Go to Dashboard
          </button>
        </div>
      </div>
    ...
    <div className="min-h-[100svh] bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
      <div className="mx-auto max-w-6xl px-3 sm:px-6 lg:px-8 pt-6 pb-10 sm:pt-10 sm:pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-7 lg:gap-6">
          <div className="lg:col-span-5"><div className="lg:sticky lg:top-6"><MethodsCard paymentMethods={paymentMethods} selectedMethod={selectedMethod} setSelectedMethod={setSelectedMethod} /></div></div>
...
function SummaryCard() { /* ...same as before... */ return (
...
function ActionCard({ isProcessing, onPay }: { isProcessing: boolean; onPay: () => void }) { /* ...same as before... */ return (
  <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-5 sm:p-8">
    ...
    <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300 mb-5 sm:mb-8">You'll be redirected to a secure payment gateway to complete your transaction.</p>
    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-3.5 sm:p-4 mb-6 sm:mb-8">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-blue-700/90 dark:text-blue-300 mt-1">Your payment details are encrypted and secure. We do not store your card information.</p>
        </div>
      </div>
    </div>
    <button onClick={onPay} disabled={isProcessing} className="w-full rounded-full bg-gradient-to-b from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold py-4 px-6 shadow-[0_6px_20px_rgba(29,78,216,0.25)] focus:outline-none focus:ring-2 focus:ring-blue-400/70 transition disabled:opacity-50 disabled:cursor-not-allowed">
...
function MethodsCard({ paymentMethods, selectedMethod, setSelectedMethod }: {
        ...
        {paymentMethods.map(m => (
          <button key={m.id} type="button"
            className={`w-full text-left p-3.5 sm:p-4 border rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              selectedMethod === m.id ? "border-blue-500 bg-blue-50 dark:bg-blue-900/20"
                                      : "border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"}`}
            onClick={() => setSelectedMethod(m.id)}>
            <div className="flex items-center">
              <div className={`p-2 rounded-lg ${selectedMethod === m.id ? "bg-blue-100 dark:bg-blue-800" : "bg-gray-100 dark:bg-gray-700"}`}>{m.icon}</div>
              <div className="ml-3 sm:ml-4">
                <h3 className="text-sm sm:text-base font-medium text-gray-900 dark:text-white">{m.name}</h3>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{m.description}</p>
              </div>
            </div>
          </button>
        ))}
