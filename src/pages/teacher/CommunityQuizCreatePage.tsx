...
// ──────────────────────────────────────────────────────────
// Community Quiz Create Wizard
//   Step 1: Paste YouTube URL → preview
//   Step 2: Configure (count, difficulty, duration window)
//   Step 3: Generating (60-90s spinner)
//   Step 4: Review questions + publish
//   Step 5: Success — share link + WhatsApp
// ──────────────────────────────────────────────────────────
...
/* ------------------- STYLES (matches TeacherDashboardPage) ------------------- */
const customStyles = `
  @keyframes fadeInUp {
  ...
  .btn-glossy-green {
    ...
    box-shadow: inset 0px 2px 4px rgba(255,255,255,0.4), 0px 8px 16px rgba(5, 150, 105, 0.3);
    border: 1px solid rgba(255,255,255,0.3);
    color: white;
  }
  ...
  .shimmer-bg {
    background: linear-gradient(90deg, rgba(59,130,246,0.05) 0%, rgba(59,130,246,0.15) 50%, rgba(59,130,246,0.05) 100%);
...
const SUBJECTS = [
  ...
  "Civics", "Economics", "Computer Science", "Accountancy",
  "Business Studies", "General Knowledge", "Other",
];
...
function isYoutubeUrl(url: string): boolean {
  const t = url.trim();
  return /^(https?:\/\/)?(www\.|m\.)?(youtube\.com|youtu\.be)\//.test(t);
}

/* ------------------- MAIN COMPONENT ------------------- */

type Step = "url" | "config" | "generating" | "review" | "success";
...
export default function CommunityQuizCreatePage() {
  ...
  // Step 3: generating timer
  const [generationStartTs, setGenerationStartTs] = useState<number | null>(null);
  const [elapsedSec, setElapsedSec] = useState(0);

  // Step 4 (skipped — we go straight to success with backend response)
  // Step 5: success
  ...
  async function handlePreview() {
    ...
    try {
      ...
      const msg = err?.message || "Could not fetch video";
      toast.error(msg);
      setPreview(null);
    } finally {
  ...
  async function handleGenerate() {
    ...
    if (!preview) {
      toast.error("Video preview missing");
      setStep("url");
      return;
    }
  ...
  /* ─────── Stepper ─────── */
  const stepNumber = { url: 1, config: 2, generating: 3, review: 3, success: 4 }[step];
    ...
    <div className={isDarkMode ? "dark" : ""}>
      <div className="min-h-[100dvh] w-full font-sans text-slate-800 dark:text-slate-100 relative bg-slate-100 dark:bg-slate-900 transition-colors duration-500 overflow-x-hidden">
        ...
        <main className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
          ...
          {/* ─── Header ─── */}
          <header className="flex items-center gap-3 sm:gap-5 mb-6 sm:mb-10 animate-entrance">
            <button
              onClick={() => navigate(-1)}
              className="p-2.5 sm:p-3 text-blue-600 inset-pill rounded-[20px] sm:rounded-[24px] border-none shrink-0 hover:scale-105 transition-transform"
            >
              <Icons.ArrowLeft />
            </button>
            <div className="min-w-0">
              ...
              <p className="text-slate-600 dark:text-blue-200/70 text-xs sm:text-sm mt-1 font-medium">
                YouTube video se quiz banao, share link generate karo
              </p>
            </div>
          </header>
              ...
              return (
                <React.Fragment key={label}>
                  <div className={`flex items-center gap-2 ${isActive || isDone ? "opacity-100" : "opacity-50"}`}>
                    <div
                      className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-black text-sm transition-all ${
                        isDone
                          ? "btn-glossy-blue"
                          : isActive
                          ? "btn-glossy-blue scale-110"
                          : "inset-pill border-none text-slate-400"
                      }`}
                    >
                      {isDone ? <Icons.Check /> : num}
                    </div>
                    <span className={`hidden sm:inline text-xs sm:text-sm font-bold ${isActive ? "text-blue-600 dark:text-blue-400" : "text-slate-500"}`}>
                      {label}
                    </span>
                  </div>
                ...
                <div className="glass-panel rounded-[28px] sm:rounded-[40px] p-5 sm:p-7 animate-pop">
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-5">
                    <img
                      ...
                      alt={preview.title}
                      className="w-full sm:w-48 h-32 sm:h-32 object-cover rounded-[20px] sm:rounded-[24px] shrink-0 border border-white/60"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/40 px-2.5 py-1 rounded-[12px] uppercase tracking-wider">
                          ✓ Loaded
                        </span>
                        <span className="text-[10px] font-bold text-slate-600 inset-pill border-none px-2.5 py-1 rounded-[12px]">
                          {preview.transcript_word_count.toLocaleString()} words
                        </span>
                      </div>
                      <h3 className="font-black text-slate-900 dark:text-white text-base sm:text-lg leading-tight mb-1 line-clamp-2">
                        {preview.title}
                      </h3>
                  ...
                  <button
                    ...
                    className="btn-glossy-blue mt-5 w-full px-5 py-3.5 rounded-[20px] font-bold text-sm flex items-center justify-center gap-2 transition-all hover:-translate-y-1 active:scale-95"
                  >
          ...
          {step === "config" && preview && (
            <div className="space-y-5 sm:space-y-6 animate-pop">
              ...
              <div className="glass-panel rounded-[28px] sm:rounded-[40px] p-6 sm:p-8 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    maxLength={100}
                    placeholder="e.g. Aman Bhaiya Weekly Quiz #5"
                    className="w-full px-4 py-3 rounded-[16px] sm:rounded-[20px] inset-pill border-none focus:outline-none focus:ring-2 focus:ring-sky-400/50 font-bold text-sm text-slate-800 dark:text-white placeholder-slate-400"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 rounded-[16px] sm:rounded-[20px] inset-pill border-none focus:outline-none focus:ring-2 focus:ring-sky-400/50 font-bold text-sm text-slate-800 dark:text-white"
                    >
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {(["easy", "medium", "hard", "mixed"] as const).map((d) => (
                      <button
                        ...
                        className={`py-2.5 sm:py-3 rounded-[14px] sm:rounded-[16px] font-black text-xs uppercase tracking-wide transition-all ${
                          difficulty === d
                            ? "btn-glossy-blue scale-105"
                            : "inset-pill border-none text-slate-600 dark:text-slate-300 hover:scale-105"
                        }`}
                      >
                        {d}
                      </button>
              ...
              <div className="flex gap-3 sm:gap-4">
                <button
                  onClick={() => setStep("url")}
                  className="btn-glossy-light px-6 py-3.5 rounded-[20px] font-bold text-sm transition-all hover:-translate-y-0.5"
                >
                  ← Back
                </button>
                <button
                  ...
                  disabled={!title.trim()}
                  className="btn-glossy-blue flex-1 px-5 py-3.5 rounded-[20px] font-bold text-sm sm:text-base flex items-center justify-center gap-2 transition-all hover:-translate-y-1 active:scale-95"
                >
                  <Icons.Sparkles />
          ...
          {step === "generating" && (
            <div className="glass-panel rounded-[32px] sm:rounded-[48px] p-8 sm:p-14 text-center animate-pop">
              ...
              <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 font-medium mb-2">
                AI video transcript se {questionCount} questions bana raha hai
              </p>
              <p className="text-xs sm:text-sm text-slate-500 mb-8">
                Usually takes 60-90 seconds. Please don't close this tab.
              </p>
              ...
              <div className="mt-8 grid grid-cols-3 gap-3 sm:gap-4 text-left">
                {[
                  ...
                  <div key={i} className={`p-3 rounded-[16px] inset-pill border-none flex items-center gap-2 transition-opacity ${s.done ? "opacity-100" : "opacity-50"}`}>
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${s.done ? "bg-emerald-500 text-white" : "bg-slate-300"}`}>
                      {s.done && <Icons.Check />}
                    </div>
            ...
            <div className="space-y-5 sm:space-y-6 animate-pop">
              <div className="glass-panel rounded-[32px] sm:rounded-[48px] p-8 sm:p-12 text-center">
                ...
                <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 font-medium mb-1">
                  {createdQuiz.total_questions} questions generated in {createdQuiz.generation_seconds}s
                </p>
                <p className="text-xs text-slate-500">
                  Share the link below — participants don't need to sign up
                </p>
              </div>
              ...
              <div className="glass-panel rounded-[28px] sm:rounded-[36px] p-5 sm:p-7">
                ...
                <div className="flex gap-2 sm:gap-3 items-center">
                  <div className="flex-1 inset-pill border-none rounded-[16px] sm:rounded-[20px] px-4 py-3 flex items-center gap-2 min-w-0">
                    <Icons.Link />
                    <code className="text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-200 truncate">
                      {createdQuiz.share_link}
                    </code>
                  </div>
                  <button
                    onClick={copyLink}
                    className="btn-glossy-blue px-4 sm:px-5 py-3 rounded-[16px] sm:rounded-[20px] font-bold text-xs sm:text-sm flex items-center gap-2 shrink-0 transition-all hover:-translate-y-0.5"
                  >
                ...
                <button
                  ...
                  className="btn-glossy-green w-full mt-4 px-5 py-3.5 rounded-[20px] font-bold text-sm sm:text-base flex items-center justify-center gap-2 transition-all hover:-translate-y-1 active:scale-95"
                >
                  <Icons.Whatsapp />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <button
                  ...
                  className="btn-glossy-light px-5 py-4 rounded-[20px] sm:rounded-[24px] font-bold text-sm flex items-center justify-center gap-2 transition-all hover:-translate-y-1"
                >
                  <Icons.Eye />
              ...
              <button
                ...
                className="w-full text-center text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors py-3"
              >
                ← Back to Dashboard
              </button>
            </div>
