...
import LegalLayout, { head, muted, accent } from "./LegalLayout";
  ...
  return (
    <LegalLayout title="Terms of Service" subtitle="Effective: Aug 2025">
      ...
      >
        ...
        <Section delay={0.35} title="1. Your Account" isDark={isDark}>
          <ul>
        ...
        <Section delay={0.4} title="2. Acceptable Use" isDark={isDark}>
          <ul>
        ...
        <Section delay={0.45} title="3. Content & IP" isDark={isDark}>
          <ul>
        ...
        <Section delay={0.5} title="4. Paid Plans" isDark={isDark}>
          <ul>
        ...
        <Section delay={0.55} title="5. Disclaimers" isDark={isDark}>
          <p>Services are provided “as is.” We do not guarantee error-free or uninterrupted operation.</p>
        </Section>

        <Section delay={0.6} title="6. Limitation of Liability" isDark={isDark}>
          <p>To the fullest extent permitted by law, a4ai shall not be liable for indirect, incidental, or consequential damages.</p>
        </Section>

        <Section delay={0.65} title="7. Termination" isDark={isDark}>
          <p>We may suspend or terminate accounts for policy or legal violations. You may stop using the service at any time.</p>
        </Section>

        <Section delay={0.7} title="8. Changes" isDark={isDark}>
          <p>We may update these Terms. Continued use after changes constitutes acceptance.</p>
        </Section>

        <Section delay={0.75} title="9. Contact" isDark={isDark}>
          <p>
            Questions? <a href="mailto:a4ai.team@gmail.com" className="font-medium hover:underline" style={{ color: accent(isDark) }}>a4ai.team@gmail.com</a>
          </p>
        </Section>
      </div>
    </LegalLayout>
...
function Section({ title, children, delay, isDark }: { title: string; children: React.ReactNode; delay: number; isDark: boolean }) {
  return (
    <motion.div
    ...
    >
      <h2 style={{ color: head(isDark) }}>{title}</h2>
      {children}
    </motion.div>
  );
}