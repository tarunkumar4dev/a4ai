...
export default function DashboardPage() {
  ...
  // Create profile if it doesn't exist (with correct role)
  useEffect(() => {
    let mounted = true;
    (async () => {
      ...
      // Get role from user metadata (set during signup) or from existing profile
      ...
      if (mounted) {
        if (error && error.code === 'PGRST116') { // Profile doesn't exist
          await supabase.from("profiles").insert({
            ...
            role: userRole,
            coins: userRole === "student" ? 100 : userRole === "teacher" ? 200 : 500,
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          });
        } else if (existing && !existing.role) {
          // Update existing profile with role if missing
          await supabase.from("profiles").update({
            role: userRole,
            coins: userRole === "student" ? 100 : userRole === "teacher" ? 200 : existing.coins || 500,
            updated_at: new Date().toISOString(),
          }).eq("id", user.id);
        }
      }
  ...
  switch (profile.role) {
    ...
    // case "institute":
    //   return <InstituteDashboardPage />; // You can create this later
    default:
      // Default to student dashboard if role is not set or unknown
      return <StudentDashboardPage />;
  }
}

/* ------------ Original Dashboard Layout (for Teacher/Student pages to extend) ------------ */
              ...
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium shrink-0">
                    {profile?.role === "student" && <GraduationCap size={14} className="sm:size-4" />}
                    {profile?.role === "teacher" && <BookOpen size={14} className="sm:size-4" />}
                    {profile?.role === "institute" && <Building size={14} className="sm:size-4" />}
                    <span className="hidden xs:inline capitalize">{profile?.role}</span>
                  </div>
  ...
  return (
    ...
    >
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                {profile?.role === "student" 
                  ? "Practice with AI-powered tests. Pick a quick action to get started."
                  : profile?.role === "teacher"
                  ? "Create curriculum-perfect tests with AI. Pick a quick action to get started."
                  : "Manage your institute's learning programs with AI tools."}
              </CardDescription>
            </div>
              ...
              <Link to="/dashboard/test-generator" className="flex-1 sm:flex-none min-w-0">
                <Button className="gap-2 w-full sm:w-auto rounded-xl bg-gray-900 text-white hover:bg-black active:scale-[.98] shadow-lg text-xs sm:text-sm">
                  ...
                  <span className="truncate">
                    {profile?.role === "student" ? "Take Practice Test" : "Create Test"}
                  </span>
                </Button>
              </Link>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        <KpiCard 
          title={profile?.role === "student" ? "Practice Tests" : "Tests Created"} 
        ...
        />
        <KpiCard 
          title={profile?.role === "student" ? "Study Time" : "Avg. Time"} 
          value="01:42" 
          icon={TimerReset} 
          tone="teal" 
          interactive={interactiveCards} 
        />
        <KpiCard 
          title={profile?.role === "student" ? "Progress" : "Syllabus"} 
          value={profile?.role === "student" ? "85%" : "99%"} 
          icon={TrendingUp} 
          tone="blue" 
          interactive={interactiveCards} 
        />
        ...
        <motion.div variants={item} className="lg:col-span-2 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-[14px] sm:text-[16px] font-[600] tracking-[-0.01em]" style={{ color: INK }}>
              {profile?.role === "student" ? "Recent Practice Tests" : "Recent Tests"}
            </h2>
        ...
        <div className="rounded-xl sm:rounded-2xl bg-card card-soft px-4 sm:px-6 py-4 sm:py-6 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs sm:text-sm text-muted-foreground">
              {profile?.role === "student"
                ? "Compete with peers, test your knowledge and win prizes."
          ...
          <Link to="/dashboard/contest/create" className="w-full sm:w-auto">
            <Button className="gap-2 w-full sm:w-auto rounded-xl bg-gray-900 text-white hover:bg-black active:scale-[.98] shadow-lg text-xs sm:text-sm">
              <Rocket className="h-3 w-3 sm:h-4 sm:w-4" />
              <span>
                {profile?.role === "student" ? "Join contest" : "Create contest"}
              </span>
            </Button>
          </Link>
        </div>
